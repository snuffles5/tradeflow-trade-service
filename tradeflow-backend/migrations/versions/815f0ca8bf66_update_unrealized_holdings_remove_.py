...
def upgrade():
    ...
    if 'open_date' not in columns:
        op.add_column('unrealized_holdings',
            sa.Column('open_date', sa.DateTime(), nullable=True, server_default=sa.text('NOW()'))
        )
    ...
    if 'entry_date' in columns:
        op.execute("UPDATE unrealized_holdings SET open_date = entry_date WHERE open_date IS NULL")
...
def downgrade():
    ...
    with op.batch_alter_table('unrealized_holdings') as batch_op:
        batch_op.alter_column('entry_date', server_default=None, existing_type=sa.DateTime())
